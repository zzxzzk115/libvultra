...
void TextEditor::SetLanguageDefinition(LanguageDefinitionId aValue)
{
	...
	case LanguageDefinitionId::Hlsl:
		...
		break;
	case LanguageDefinitionId::VultraShader:
		mLanguageDefinition = &(LanguageDefinition::VultraShader());
		break;
	}
...
bool TextEditor::Render(const char* aTitle, bool aParentIsFocused, const ImVec2& aSize, bool aBorder)
{
	...
	HandleMouseInputs();
	HandleKeyboardInputs(isFocused || aParentIsFocused);
	ColorizeInternal();
				...
				if (focused)
				{
					for (const auto& cursorCoords : cursorCoordsInThisLine)
					{
						...
						if (!mReadOnly)
						{
							...
							ImGuiPlatformImeData* imeData = &g.PlatformImeData;
							imeData->WantVisible = true;
							imeData->WantTextInput = true;
							imeData->InputPos = ImVec2(cstart.x - 1.0f, cstart.y);
							imeData->InputLineHeight = ImGui::GetFontSize();
