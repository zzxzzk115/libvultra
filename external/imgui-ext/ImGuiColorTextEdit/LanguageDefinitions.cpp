...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::VultraShader()
{
	static bool inited = false;
	static LanguageDefinition langDef;
	if (!inited)
	{
		static const char* const keywords[] = {
			"bool", "break", "case", "const", "continue", "default", "discard", "do", "else", "false",
			"float", "for", "if", "in", "inout", "int", "layout", "mat2", "mat3", "mat4", "out",
			"return", "sampler2D", "sampler2DArray", "sampler3D", "samplerCube", "struct", "switch",
			"true", "uint", "uniform", "vec2", "vec3", "vec4", "void", "while"
		};
		for (auto& k : keywords)
			langDef.mKeywords.insert(k);

		static const char* const identifiers[] = {
			"abs", "acos", "asin", "atan", "ceil", "clamp", "cos", "cross", "dFdx", "dFdy", "distance",
			"dot", "exp", "floor", "fract", "length", "log", "max", "min", "mix", "normalize", "pow",
			"reflect", "refract", "sin", "smoothstep", "sqrt", "step", "tan", "texture", "textureLod",
			"language", "profile", "version", "entry", "stage", "vertex", "fragment", "compute", "glsl",
			"hlsl", "wgsl", "push_constant", "set", "binding", "location", "descriptor", "variant"
		};
		for (auto& k : identifiers)
		{
			Identifier id;
			id.mDeclaration = "Vultra shader DSL";
			langDef.mIdentifiers.insert(std::make_pair(std::string(k), id));
		}

		langDef.mTokenRegexStrings.push_back(std::make_pair<std::string, PaletteIndex>(R"##(\[(vshader|vert|frag|comp|vertex|fragment|compute)\])##", PaletteIndex::Preprocessor));
		langDef.mTokenRegexStrings.push_back(std::make_pair<std::string, PaletteIndex>(R"##([ \t]*#[ \t]*[a-zA-Z_]+)##", PaletteIndex::Preprocessor));
		langDef.mTokenRegexStrings.push_back(std::make_pair<std::string, PaletteIndex>(R"##(L?\"(\\.|[^\"])*\")##", PaletteIndex::String));
		langDef.mTokenRegexStrings.push_back(std::make_pair<std::string, PaletteIndex>(R"##(\'\\?[^\']\')##", PaletteIndex::CharLiteral));
		langDef.mTokenRegexStrings.push_back(std::make_pair<std::string, PaletteIndex>(R"##([+-]?([0-9]+([.][0-9]*)?|[.][0-9]+)([eE][+-]?[0-9]+)?[fF]?)##", PaletteIndex::Number));
