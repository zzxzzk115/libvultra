...
class MeshShadingSponzaApp final : public ImGuiApp
{
public:
    explicit MeshShadingSponzaApp(const std::span<char*>& args) :
        ImGuiApp(
            args,
            {.title = "MeshShading Sponza", .renderDeviceFeatureFlag = rhi::RenderDeviceFeatureFlagBits::eMeshShader},
            {.enableDocking = false}),
        m_Renderer(*m_RenderDevice, m_Swapchain.getFormat())
    {
        ...
        // Use Mesh Shading Renderer
        m_Renderer.getSettings().rendererType = gfx::RendererType::eMeshShading;
    ...
    {
        ImGui::Begin("MeshShading Sponza Example");
...
CONFIG_MAIN(MeshShadingSponzaApp)