...
layout(set = 0, binding = 0) uniform samplerCube v_EnvMap;

layout(push_constant) uniform _PushConstants { float roughness; };
...
void main() {
    const float width = textureSize(v_EnvMap, 0).x;
    ...
    for(uint i = 0; i < kNumSamples; ++i) {
        ...
        if(NdotL > 0.0) {
            ...
            if(roughness == 0.0)
                lod = 0.0;
            const vec3 sampleColor = textureLod(v_EnvMap, L, lod).rgb;
