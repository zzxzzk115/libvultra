...
namespace vultra
{
    namespace rhi
    {
        ...
        class VulkanDescriptorSetBuilder final : public IDescriptorSetBuilder
        {
        public:
            using Cache = std::unordered_map<std::size_t, DescriptorSetHandle>;

            VulkanDescriptorSetBuilder(const RenderDevice&, std::uintptr_t deviceHandle, DescriptorSetAllocator&, Cache&);
