...
namespace vultra
{
    void registerScriptScriptBindings(sol::state& lua, ScriptContext& ctx)
    {
        ...
        script.set_function("reloadAll", [&ctx]() { return ctx.scriptService ? ctx.scriptService->reloadAllScripts() : false; });
        script.set_function("reloadEntity", [&ctx](const ScriptEntity& entity) {
            return ctx.scriptService && ctx.isValid(entity.value) ? ctx.scriptService->reloadEntityScript(entity.value) : false;
        });
        script.set_function("hasInstance", [&ctx](const ScriptEntity& entity) {
            return ctx.scriptService && ctx.isValid(entity.value) ? ctx.scriptService->hasScriptInstance(entity.value) : false;
        });
        ...
        script.set_function("isPlaybackPlaying", [&ctx]() {
            return ctx.scriptService ? ctx.scriptService->isPlaybackPlaying() : false;
        });
        script.set_function("isPlaybackPaused", [&ctx]() {
