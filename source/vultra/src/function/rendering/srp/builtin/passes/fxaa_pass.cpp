...
namespace vultra
{
    ...
    FrameGraphResource FxaaPass::addPass(FrameGraphBuildContext& ctx, FrameGraphResource source)
    {
        ...
        const auto data = ctx.fg.addCallbackPass<PassData>(
            PASS_NAME,
            [source, outputDesc = makeInheritedTextureDesc(sourceDesc, format)](FrameGraph::Builder& builder, PassData& pd) {
                PASS_SETUP_ZONE;
    ...
    rhi::GraphicsPipeline FxaaPass::createPipeline(const rhi::PixelFormat colorFormat, const uint32_t viewMask) const
    {
        ...
        auto fragmentShader = loadGeneralShader("fxaa.frag", vshadersystem::ShaderStage::eFrag, fragmentKeywords);
        if (!fragmentShader)
        {
