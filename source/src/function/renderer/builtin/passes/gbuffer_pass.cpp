...
#include "gbuffer_alpha_masking.frag.spv.h"
#include "geometry.vert.spv.h"
...
namespace vultra
{
    ...
    struct MeshConstants
    {
        ...
        MeshConstants(const glm::mat4& m, const gfx::PBRMaterial& mat) :
            ...
            useMetallicTexture(mat.useMetallicTexture ? 1 : 0), useRoughnessTexture(mat.useRoughnessTexture ? 1 : 0),
            useNormalTexture(mat.useNormalTexture ? 1 : 0), useAOTexture(mat.useAOTexture ? 1 : 0),
            useEmissiveTexture(mat.useEmissiveTexture ? 1 : 0),
        ...
        const auto& gBufferData = fg.addCallbackPass<GBufferData>(
            ...
            [this, renderPrimitiveGroup, enableAreaLight](const GBufferData&, auto&, void* ctx) {
                ...
                for (const auto& primitive : opaquePrimitives)
                {
                    ...
                    const auto* pipeline = getPipeline(passInfo, primitive.renderSubMesh.material.doubleSided, true);

                    cb.bindPipeline(*pipeline).pushConstants(rhi::ShaderStages::eVertex | rhi::ShaderStages::eFragment,
                                                             0,
                                                             sizeof(MeshConstants),
                                                             &meshConstants);

                    rc.resourceSet[3][0] =
                        rhi::bindings::CombinedImageSampler {.texture = primitive.renderSubMesh.material.albedo.get(),
                                                             .imageAspect = rhi::ImageAspect::eColor};
                    rc.resourceSet[3][1] =
                        rhi::bindings::CombinedImageSampler {.texture = primitive.renderSubMesh.material.metallic.get(),
                                                             .imageAspect = rhi::ImageAspect::eColor};
                    rc.resourceSet[3][2] = rhi::bindings::CombinedImageSampler {
                        .texture     = primitive.renderSubMesh.material.roughness.get(),
                        .imageAspect = rhi::ImageAspect::eColor};
                    rc.resourceSet[3][3] =
                        rhi::bindings::CombinedImageSampler {.texture = primitive.renderSubMesh.material.normal.get(),
                                                             .imageAspect = rhi::ImageAspect::eColor};
                    rc.resourceSet[3][4] = rhi::bindings::CombinedImageSampler {
                        .texture = primitive.renderSubMesh.material.ao.get(), .imageAspect = rhi::ImageAspect::eColor};
                    rc.resourceSet[3][5] =
                        rhi::bindings::CombinedImageSampler {.texture = primitive.renderSubMesh.material.emissive.get(),
                                                             .imageAspect = rhi::ImageAspect::eColor};
                    rc.resourceSet[3][6] = rhi::bindings::CombinedImageSampler {
                        .texture     = primitive.renderSubMesh.material.metallicRoughness.get(),
                        .imageAspect = rhi::ImageAspect::eColor};
                    rc.resourceSet[3][7] =
                        rhi::bindings::CombinedImageSampler {.texture = primitive.renderSubMesh.material.specular.get(),
                                                             .imageAspect = rhi::ImageAspect::eColor};
                ...
                if (enableAreaLight)
                {
                    ...
                    {
                        rhi::ShaderStageInfo vs {
                            .code = os::FileSystem::readFileAllText("assets/shaders/area_light_debug.vert")};
                        rhi::ShaderStageInfo fs {
                            .code = os::FileSystem::readFileAllText("assets/shaders/area_light_debug.frag")};
                        ...
                        builder.setDepthFormat(passInfo.depthFormat)
                            ...
                            .addShader(rhi::ShaderType::eVertex, vs)
                            .addShader(rhi::ShaderType::eFragment, fs)
                            .setDepthStencil(
                                {.depthTest = true, .depthWrite = true, .depthCompareOp = rhi::CompareOp::eLessOrEqual})
                ...
                for (const auto& primitive : decalPrimitives)
                {
                    ...
                    if (!m_DecalPipelineCreated)
                    {
                        ...
                        builder.setDepthFormat(passInfo.depthFormat)
                            ...
                            .setRasterizer({.polygonMode = rhi::PolygonMode::eFill, .cullMode = rhi::CullMode::eBack});
                        for (auto i = 0; i < passInfo.colorFormats.size(); ++i)
                        {
    ...
    GBufferPass::createPipeline(const gfx::BaseGeometryPassInfo& passInfo, bool doubleSided, bool alphaMasking) const
    {
