...
namespace vultra
{
    namespace rhi
    {
        ...
        VulkanSwapchainBackend::VulkanSwapchainBackend(const std::uintptr_t instance,
        ...
        VulkanSwapchainBackend::~VulkanSwapchainBackend() { destroy(); }

        bool VulkanSwapchainBackend::isValid() const { return m_Handle != nullptr; }

        SwapchainFormat VulkanSwapchainBackend::getFormat() const { return m_Format; }

        PixelFormat VulkanSwapchainBackend::getPixelFormat() const
        {
            assert(m_Handle && !m_Buffers.empty());
            return m_Buffers.back().getPixelFormat();
        }

        Extent2D VulkanSwapchainBackend::getExtent() const
        {
            assert(m_Handle && !m_Buffers.empty());
            return m_Buffers.back().getExtent();
        }

        std::size_t VulkanSwapchainBackend::getNumBuffers() const { return m_Buffers.size(); }

        std::uintptr_t VulkanSwapchainBackend::getHandle() const
        {
            assert(m_Handle);
            return reinterpret_cast<std::uintptr_t>(static_cast<VkSwapchainKHR>(m_Handle));
        }

        const std::vector<Texture>& VulkanSwapchainBackend::getBuffers() const { return m_Buffers; }

        const Texture& VulkanSwapchainBackend::getBuffer(const uint32_t index) const { return m_Buffers[index]; }

        uint32_t VulkanSwapchainBackend::getCurrentBufferIndex() const { return m_CurrentImageIndex; }

        Texture& VulkanSwapchainBackend::getCurrentBuffer() { return m_Buffers[m_CurrentImageIndex]; }

        void VulkanSwapchainBackend::recreate(const std::optional<VerticalSync> vsync)
        {
            m_Buffers.clear();
            createSwapchain(m_Format, vsync.value_or(m_VerticalSync));
        }

        bool VulkanSwapchainBackend::acquireNextImage(const std::uintptr_t imageAcquired)
        {
            assert(m_Handle);
        ...
        void VulkanSwapchainBackend::createSurface()
        {
            assert(m_Instance);
        ...
        void VulkanSwapchainBackend::buildBuffers(const Extent2D extent, const PixelFormat pixelFormat)
        {
            ...
            {
                m_Buffers.emplace_back(Texture::fromExternalImage(
        ...
        void VulkanSwapchainBackend::createSwapchain(const SwapchainFormat format, const VerticalSync vsync)
        {
            m_Device.waitIdle();
        ...
        void VulkanSwapchainBackend::destroy()
        {
            m_Buffers.clear();
        ...
        std::shared_ptr<ISwapchainBackend> createSwapchainBackend(const std::uintptr_t instance,
                                                                  const std::uintptr_t physicalDevice,
                                                                  const std::uintptr_t device,
                                                                  os::Window*          window,
                                                                  const SwapchainFormat format,
                                                                  const VerticalSync   vsync)
        {
            return std::make_shared<VulkanSwapchainBackend>(instance, physicalDevice, device, window, format, vsync);
        }
    } // namespace rhi
} // namespace vultra
