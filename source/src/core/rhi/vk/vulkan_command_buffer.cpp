...
namespace vultra
{
    namespace rhi
    {
        ...
        [[nodiscard]] vk::ImageAspectFlags toVk(const ImageAspectFlags aspectMask)
        {
...
#define TRACY_GPU_ZONE2_(Label) TRACY_GPU_ZONE_(m_TracyContext, m_Handle, "RHI::" Label)
        ...
        vk::CommandBuffer VulkanCommandBuffer::getHandle() const { return m_Handle; }
        ...
        TracyVkCtx VulkanCommandBuffer::getTracyContext() const { return m_TracyContext; }
        ...
        VulkanCommandBuffer& VulkanCommandBuffer::submit(const JobInfo& jobInfo, const bool oneTime)
        {
            ...
            if (oneTime)
            {
                VK_CHECK(m_Device.waitForFences(m_Fence, VK_TRUE, UINT64_MAX),
                         "VulkanCommandBuffer",
                         "Failed to wait for fence");
        ...
        VulkanCommandBuffer& VulkanCommandBuffer::bindPipeline(const BasePipeline& pipeline)
        {
            ...
            if (m_Pipeline != &pipeline)
            {
                ...
                m_Handle.bindPipeline(static_cast<vk::PipelineBindPoint>(pipeline.getBindPoint()),
        ...
        VulkanCommandBuffer& VulkanCommandBuffer::dispatchIndirect(const Buffer& buffer, const vk::DeviceSize offset)
        {
            assert(buffer);
            assert(invariant(State::eRecording, InvariantFlags::eValidComputePipeline));
            ...
            flushBarriers();
            m_Handle.dispatchIndirect(vk::Buffer {reinterpret_cast<VkBuffer>(buffer.getNativeHandle())}, offset);
                                                        ...
                                                        const vk::DescriptorSet  descriptorSet)
        {
            ...
            m_Handle.bindDescriptorSets(static_cast<vk::PipelineBindPoint>(m_Pipeline->getBindPoint()),
                                        vk::PipelineLayout {
        ...
        VulkanCommandBuffer&
        VulkanCommandBuffer::copyBuffer(const Buffer& src, Texture& dst, std::span<const vk::BufferImageCopy> copyRegions)
        {
        ...
        VulkanCommandBuffer&
        VulkanCommandBuffer::update(Buffer& buffer, const vk::DeviceSize offset, const vk::DeviceSize size, const void* data)
        {
            assert(buffer && data);
            assert(invariant(State::eRecording, InvariantFlags::eOutsideRenderPass));
            ...
            {
                chunkedUpdate(reinterpret_cast<VkBuffer>(buffer.getNativeHandle()), offset, size, data);
            }
            else
            {
                m_Handle.updateBuffer(reinterpret_cast<VkBuffer>(buffer.getNativeHandle()), offset, size, data);
            }
            return *this;
        }
                ...
                .imageBarrier(
                    {
                        ...
                        .subresourceRange =
                            ImageSubresourceRange {
                                ...
                                .levelCount     = 1u,
                                .baseArrayLayer = src.m_BaseArrayLayer,
                                .layerCount     = 1u,
                            },
                    },
                    {
                ...
                .imageBarrier(
                    {
                        ...
                        .subresourceRange =
                            ImageSubresourceRange {
                                ...
                                .levelCount     = 1u,
                                .baseArrayLayer = dst.m_BaseArrayLayer,
                                .layerCount     = 1u,
                            },
                    },
                    {
        ...
        VulkanCommandBuffer& VulkanCommandBuffer::flushBarriers()
        {
            ...
            if (auto barrier = m_BarrierBuilder.build(); barrier.isEffective())
            {
                ...
                memoryBarriers.reserve(barrier.m_MemoryBarriers.size());
                for (const auto& memory : barrier.m_MemoryBarriers)
                {
                ...
                bufferBarriers.reserve(barrier.m_BufferBarriers.size());
                for (const auto& buffer : barrier.m_BufferBarriers)
                {
                ...
                imageBarriers.reserve(barrier.m_ImageBarriers.size());
                for (const auto& image : barrier.m_ImageBarriers)
                {
        ...
        VulkanCommandBuffer::VulkanCommandBuffer(const vk::Device        device,
            ...
            m_DescriptorSetAllocator(device, enableRaytracing),
        ...
        bool VulkanCommandBuffer::invariant(const State requiredState, const InvariantFlags flags) const
        {
            ...
            {
                assert(m_Pipeline);
                valid |= m_Pipeline->getBindPoint() == PipelineBindPoint::eGraphics;
            }
            if (static_cast<bool>(flags & InvariantFlags::eComputePipeline))
            {
                assert(m_Pipeline);
                valid |= m_Pipeline->getBindPoint() == PipelineBindPoint::eCompute;
            }
            if (static_cast<bool>(flags & InvariantFlags::eRayTracingPipeline))
            {
                assert(m_Pipeline);
                valid |= m_Pipeline->getBindPoint() == PipelineBindPoint::eRayTracing;
            }
